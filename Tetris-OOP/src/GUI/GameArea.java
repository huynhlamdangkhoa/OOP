...
//        arrayBlock[3] = blocks[random.nextInt(blocks.length)];
    ...
    public void drawGhostBlock(Graphics2D g2){
        int Row = block.getRow();
        int Column = block.getColumn();
        int[][] shape = block.getShape();
        BufferedImage image = block.Image();
        ...
        for(int lowest = block.getRow(); lowest < Constant.MAX_SCREEN_ROW; lowest++) {
            if(CollisionCheck.checkBottom(block) == true) {
                continue;
            }
            ...
            for (int row = 0; row < Row; row++){
                for (int column = 0; column < Column; column++){
                    if (shape[row][column] == 1){
                        int x = Constant.CENTER + (block.getX() + column) * Constant.GridCellSide;
                        int y = lowest + Constant.MAX_SCREEN_ROW * Constant.GridCellSide;
